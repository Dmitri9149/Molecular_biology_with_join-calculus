...
if [ "x$NOJOIN" = "x" ]; then NOJOIN="-nojoin"; fi
if [ "x$PREFIX" = "x" ]; then PREFIX="/usr/local"; fi
if [ "x$BINDIR" = "x" ]; then BINDIR="${PREFIX}/bin"; fi
if [ "x$LIBDIR" = "x" ]; then LIBDIR="${PREFIX}/lib/jocaml"; fi
...
if [ "x$SYSTEM" = "x" ]; then SYSTEM="linux"; fi
...
if [ "x$OTHERLIBRARIES" = "x" ]; then OTHERLIBRARIES="unix dynlink bigarray threads systhreads join"; fi
if [ "x$DEBUGGER" = "x" ]; then DEBUGGER=""; fi
...
if [ "x$MKEXEDEBUGFLAG" = "x" ]; then MKEXEDEBUGFLAG="-g"; fi
if [ "x$MKDLL" = "x" ]; then MKDLL="gcc -shared"; fi
if [ "x$MKMAINDLL" = "x" ]; then MKMAINDLL="gcc -shared"; fi
if [ "x$RUNTIMED" = "x" ]; then RUNTIMED="noruntimed"; fi
if [ "x$CAMLP4" = "x" ]; then CAMLP4=""; fi
if [ "x$ASM_CFI_SUPPORTED" = "x" ]; then ASM_CFI_SUPPORTED="true"; fi
if [ "x$WITH_FRAME_POINTERS" = "x" ]; then WITH_FRAME_POINTERS="false"; fi
WINDOWS=false
